...
def embed_sentences(
    ...
    embeddings, texts = [], []
    save_threshold = [
        i * SAVE_EVERY for i in range(1, samples // SAVE_EVERY + 2)]
    for i in tqdm.trange(0, len(sentences), bsize):
        ...
        if save_dir is not None and finished_count > save_threshold[0]:
            ...
            np.save(os.path.join(
                save_dir, f'{finished_count}.npy'), embeddings)
            json.dump(texts, open(os.path.join(
                save_dir, f'{finished_count}.json'), 'w'))
            save_threshold.pop(0)
            embeddings = []
            texts = []
    if len(embeddings) > 0:
        np.save(os.path.join(save_dir, f'{finished_count}.npy'), np.concatenate(
            embeddings, axis=0))
        json.dump(texts, open(os.path.join(
            save_dir, f'{finished_count}.json'), 'w'))
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        embeddings = embed_sentences(
            embed_func, data, samples, save_dir=save_dir)
