...
def remove_empty(samples: Union[List[str], List[Tuple]]) -> List[str]:
    ...
    s = samples[0]
    if isinstance(s, str): return [s for s in samples if s.strip()]
    return [s for s in samples if s[0].strip()]

def split_delimiter(snippets: List[str], delimiter: str = '\n') -> List[Tuple[str, float]]:
    """
...
def split_delimiter_(snippet: str, delimiter: str = '\n'):
    ...
    for split_snip in split_snips:
        ...
        if weight: new_snippets.append((split_snip, weight/total_weight))
...
def split_truncate(
    ...
    if isinstance(weighted_snippets[0], str): weighted_snippets = give_full_weight(weighted_snippets)
    return list(chain(*[split_truncate_(w_s, cap=cap) for w_s in weighted_snippets]))

def split_truncate_(weighted_snippet: Tuple[str, float], cap: int = SNIPPET_MAXLEN) -> List[Tuple[str, float]]:
    """
    Splits any snippet over the truncation limit and returns
    new snippets weighted by word count.
    """
    if isinstance(weighted_snippet, str): weighted_snippet = (weighted_snippet, 1)
...
def split_truncate_word(
    ...
    if isinstance(weighted_snippets[0], str): weighted_snippets = give_full_weight(weighted_snippets)
    return list(chain(*[split_truncate_word_(w_s, cap=cap) for w_s in weighted_snippets]))

def split_truncate_word_(weighted_snippet: Tuple[str, float], cap: int = SNIPPET_MAXLEN):
    """
